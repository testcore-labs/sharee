...
class twig {
    ...
    private static function twig() {
        ...
        $twig->addFilter(new \Twig\TwigFilter('int_shorten', function ($number, $precision = 3, $divisors = null) {
            return int_shorten((int)$number, $precision, $divisors);
        }));
        $twig->addFilter(new \Twig\TwigFilter('int_shorten_size', function ($number, $precision = 3) {
          return int_shorten_size((int)$number, $precision);
        }));
        ...
        $twig->addFunction(new \Twig\TwigFunction('ipua2identifier', function () {
            return ipua2identifier();
        }));
        $twig->addFunction(new \Twig\TwigFunction('get_timezone', function () {
        ...
        $twig->addFunction(new \Twig\TwigFunction('identifierto360deg', function ($identifier) {
            return identifierto360deg($identifier);
        }));
        $twig->addFilter(new \Twig\TwigFilter('timeago', function ($int = 0) {
            return @timeago((int)$int);
        }));
        $twig->addFilter(new \Twig\TwigFilter('emojitohtml', function ($text, $size = "s") {
            return @emojitohtml($text, $size, $emojiss == null ? $emojis : $emojiss);
        }));
        $twig->addFilter(new \Twig\TwigFilter('strtotime', function ($str) {
            return strtotime($str);
        }));
        $twig->addFilter(new \Twig\TwigFilter('duration_format', function ($int = 0) {
            return @duration_format($int);
        }));
    ...
    public static function view($file, $data = []) {
        ...
        $data = array_merge($data, self::$variables);
        try {
